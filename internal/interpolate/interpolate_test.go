...
func TestApplyURLEscapesQueryValues(t *testing.T) {
	got, missing := ApplyURL("http://localhost:8080/get?greeting={{greeting}}", map[string]string{"greeting": "Hello again"})
...
func TestApplyURLLeavesHostPrefixVariableRaw(t *testing.T) {
	...
	got, missing := ApplyURL("{{host}}/get?greeting={{greeting}}", map[string]string{
...
func TestApplyURLLeavesPathSegmentVariableRaw(t *testing.T) {
	got, _ := ApplyURL("http://localhost:8080/users/{{id}}", map[string]string{"id": "abc-123"})
...
func TestApplyURLEscapesMultipleQueryParams(t *testing.T) {
	got, _ := ApplyURL("http://localhost:8080/get?a={{a}}&b={{b}}", map[string]string{"a": "x y", "b": "1&2"})
...
func TestApplyURLLeavesValuelessQueryFlagRaw(t *testing.T) {
	got, _ := ApplyURL("http://localhost:8080/get?{{flag}}", map[string]string{"flag": "debug"})
...
func TestApplyURLNoQueryString(t *testing.T) {
	got, missing := ApplyURL("{{host}}/get", map[string]string{"host": "http://localhost:8080"})
...
func TestApplyURLMissingQueryValueLeftUntouchedAndReported(t *testing.T) {
	got, missing := ApplyURL("http://localhost:8080/get?token={{token}}", nil)
...
func TestApplyURLDedupesMissingAcrossBaseAndQuery(t *testing.T) {
	_, missing := ApplyURL("http://localhost:8080/{{x}}?a={{x}}", nil)
